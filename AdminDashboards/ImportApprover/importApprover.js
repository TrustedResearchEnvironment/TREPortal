...
const GET_ALL_IMPORT_REQUESTS = 'GetAllImportRequests';
const API_APPROVE_REQUEST = 'ApproveImportRequest';
const API_APPROVE_REQ_INTEGRATE_JOB = 'ApprovedImportRequestIntegrateJob';
const API_REJECT_REQUEST = 'RejectImportRequest';
const API_GET_REQUEST_DETAILS = 'GetImportRequestByID';
...
function ApproveRequest(request) {
    ...
    modalTitle.textContent = `Approve Request: ${request.ImportRequestName}`;
    ...
    if (confirmBtn) {
        confirmBtn.addEventListener('click', () => {
            ...
            approveRequestFromAPI(request.ImportRequestID, note);
        });
    }
}
    ...
    if (data.length === 0) {
        ...
        data.forEach(item => {
            ...
            row.addEventListener('click', async () => {
                // console.log('Row clicked:', item.ImportRequestID);
                ...
                if (!accordionRow.classList.contains('hidden')) {
                    const combinedDetailsContainer = accordionRow.querySelector(`#combined-details-${item.ImportRequestID}`);
                    
                    // Show loading indicator
                    combinedDetailsContainer.innerHTML = '<p class="text-center">Loading details...</p>';
                    
                    try {
                        // console.log(`Fetching details for RequestID: ${item.ImportRequestID}`);
                        ...
                        combinedDetailsContainer.querySelector('.retry-btn')?.addEventListener('click', async (e) => {
                            ...
                            combinedDetailsContainer.innerHTML = '<p class="text-center">Loading details...</p>';
                            try {
                                const retryRequestDetails = await fetchRequestDetails(item.ImportRequestID);
...
function showToast(message, type = 'info') {
    ...
    toastContainer.style.cssText = 'position:fixed;top:12px;right:12px;z-index:9999;display:flex;flex-direction:column;gap:8px;';
